...
public class EditTextLocker {
    ...
    private TextWatcher editTextWatcherForMaxLimit = new TextWatcher() {

        @Override
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            double editTextLength = 0;
            if (editText.length() != 0) {
                editTextLength = Double.parseDouble(editText.getText().toString());
            }
                if (editTextLength < max) {
                    startStopEditing(true);
                } else {
                    editText.setText("");
                }
    ...
    public void maxTextInput(int max){
        this.max = max;
        editText.addTextChangedListener(editTextWatcherForMaxLimit);
    }
